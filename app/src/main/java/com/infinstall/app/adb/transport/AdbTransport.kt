...
class AdbTransport(
    ...
    /**
     * Single official install step 2/3: `adb shell pm install …`
     * Destination must stay short (libadb OPEN ≤~104 bytes) — use /data/local/tmp short names.
     */
    fun pmInstall(remoteApkPath: String, timeoutMs: Long = 90_000): String {
        ensureLive()
        // Flags match common `adb install -r -t -g` (+ -d for test flexibility)
        val cmd = "pm install -r -t -d -g $remoteApkPath"
        val dest = "shell:$cmd"
        Log.i(TAG, "pmInstall destLen=${dest.length} $cmd")
        return if (dest.length < 100) {
            // One-shot shell:service — same as `adb shell pm install …`, exits when pm done
            timed(timeoutMs) {
                var stream: AdbStream? = null
                try {
            ...
            // Path unexpectedly long: interactive shell: (OPEN dest is only "shell:")
            shell("$cmd; echo __EC:\$?", timeoutMs)
        }
    }
